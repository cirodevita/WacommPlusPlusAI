...
def worker(year, month, day, hours, lat, long, delta_lat, delta_long, max_measures, area):
    bbox = area['bbox']
    min_long = bbox[0]
    min_lat = bbox[1]
    max_long = bbox[2]
    ...
    for i in range(0, hours + 1):
        reference_hour = datetime(year, month, day, 0, 0) + timedelta(hours=i)
...
def create_timeseries(lat, long, delta_lat, delta_long, max_measures, areas):
    ...
    day = cfg.getint('time_series', 'DAY')
    end_year = cfg.getint('time_series', 'END_YEAR')
    end_month = cfg.getint('time_series', 'END_MONTH')
    end_day = cfg.getint('time_series', 'END_DAY')
    dt = datetime(year, month, day, 00, 00, 00)
    dt2 = datetime(end_year, end_month, end_day, 00, 00, 00)
    hours = (dt2 - dt).days * 24

    time_series = []

    workers = multiprocessing.cpu_count()
    func = partial(worker, year, month, day, hours, lat, long, delta_lat, delta_long, max_measures)
    ...
    for time in time_series:
        ...
        for t in time['datetime']:
            if temp_month == int(t.month):
                ...
                if len(x1) != 0:
                    for k in x1[::-1]:
                        plt.fill_between(x, 0, y, color='yellow',
                                         where=(x < k - timedelta(hours=72)) & (x > k - timedelta(hours=168)))
